...
#(sequência: 0000, 0001, 0011, 0111, 1111, 1110, 1100,
#1000, 0000, 0001, ...), com uma frequência de 
#atualização de 1.5 Hz; para implementar este contador
#observe que o bit a introduzir na posição menos 
#significativa quando se faz o deslocamento à esquerda
#corresponde ao valor negado que o bit mais
#significativo tinha na iteração anterior.
...
while:  lw $t1, LATE($s0)               #ler valor do LATE 
	...
	jal delay			#delay(667) -> 1.5 Hz [1/(1.5 Hz) = 0.6667 s]
        ...
        andi $t2, $t1, 0x0008           #$t2 = RE[3]
        srl $t2,$t2, 0x0003             #LSB 
        sll $s1,$s1, 1                  #count = count << 1
        xori $t2,$t2, 0x0001            #$t2 = RE3\
        or $s1,$s1, $t2                 #fisrt bit of count
